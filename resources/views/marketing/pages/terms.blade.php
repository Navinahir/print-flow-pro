...
@section('title', __('marketing.legal.terms.title').' — '.__('marketing.brand.page_title_suffix'))

@section('content')
    <section class="w-full px-margin-mobile md:px-margin-desktop py-12 max-w-container-max mx-auto">
        <h1 class="text-3xl font-bold text-on-background mb-6">{{ __('marketing.legal.terms.title') }}</h1>
        <div class="prose prose-slate max-w-3xl dark:prose-invert space-y-4">
            @foreach (__('marketing.legal.terms.sections') as $section)
                <div>
