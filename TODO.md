...
| Item | Status | Notes |
| --- | --- | --- |
| `PickingListSpreadsheetReader` (Maatwebsite Excel) | [PLANNED] | Parse CSV/XLS/XLSX from temp |
| `PickingListAggregationService` — `groupBy()->sum()` | [PLANNED] | product + variant key |
| Create `PickingList` model rows on import | [PLANNED] | Populate `row_count`, metadata |
| `PickingListProcessor` + PDF output | [PLANNED] | Single merged picking sheet |
| Wire `PickingListPreviewService` to aggregated data | [PLANNED] | Replace samples |

---

## M2 — Phase 6: Order Details processor

| Item | Status | Notes |
| --- | --- | --- |
| `OrderDetailsProcessor` | [PLANNED] | Merge and/or HTML render |
| PDF text/table extraction strategy (if required) | [PENDING] | Product decision: merge-only vs parse |
| Streamed download (privacy) | [PLANNED] | V2.4 stateless stream preference |
| Wire `OrderDetailsPreviewService` to real data | [PLANNED] | Replace samples |
